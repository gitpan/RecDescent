...
This document refers to RecDescent version 1.24.
...
result of mispellings, and is a sufficiently common occurance that a
warning is generated for such situations.

However, when prototyping a grammar it is sometimes useful to use
subrules before a proper specification of them is possible. For
example, a grammar might include a section like:
...
A warning is issued for each such "autostubbed" rule.
...
or with the explicit C<m<delimiterE<gt>......<delimiterE<gt>> syntax:
...
Assignment to C<$thisline> has no effect.
...
C<$::RD_ERROR>, C<$::RD_WARN>, C<$::RD_HINT>, and
...
cause the failure of a production (a warning only - see
